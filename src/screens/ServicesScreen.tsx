...
export default function ServicesScreen({ barberId, onBack }: { barberId: string; onBack?: () => void }) {
  ...
  async function load() {
    ...
    if (error) Alert.alert(tr('Could not load services'), error.message);
  ...
  async function save() {
    ...
    if (!name.trim() || !Number.isFinite(priceCents) || priceCents < 0 || !Number.isInteger(durationMin) || durationMin <= 0) {
      return Alert.alert(tr('Invalid input'), tr('Name, a valid price, and a duration in minutes are required.'));
    }
    setBusy(true);
  ...
  return (
    <View style={s.screen}>
      <View style={s.head}>
        <TopBar title={editingId ? tr('Edit service') : tr('My services')} onBack={onBack} plain />
        <View style={s.addCard}>
          <Eyebrow ls={1.4}>{editingId ? tr('EDIT SERVICE') : tr('ADD A SERVICE')}</Eyebrow>
          <TextInput value={name} onChangeText={setName} style={s.field}
            placeholder={tr('Service name (e.g. Haircut)')} placeholderTextColor={D.sub}
            accessibilityLabel={tr('Service name')} />
          <View style={s.fieldRow}>
            <TextInput value={price} onChangeText={setPrice} keyboardType="decimal-pad"
              style={[s.field, s.grow]} placeholder={tr('Price (DH)')} placeholderTextColor={D.sub}
              accessibilityLabel={tr('Price in dirhams')} />
            <TextInput value={duration} onChangeText={setDuration} keyboardType="number-pad"
              style={[s.field, s.grow]} placeholder={tr('Minutes')} placeholderTextColor={D.sub}
              accessibilityLabel={tr('Duration in minutes')} />
          </View>
          <Btn title={editingId ? tr('UPDATE SERVICE') : tr('ADD SERVICE')} height={48} onPress={save}
            style={busy ? { opacity: 0.6 } : undefined} />
          {editingId && (
            <Pressable onPress={clearForm} accessibilityRole="button"
              style={({ pressed }) => pressed && s.pressed}>
              <T w="sb" size={12} c={D.sub} style={s.center}>{tr('Cancel edit')}</T>
            </Pressable>
          )}
        </View>
        <Eyebrow ls={1.65}>
          {trn(services.length, '{n} SERVICE · TAP TO EDIT', '{n} SERVICES · TAP TO EDIT')}
        </Eyebrow>
      </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <T size={13} c={D.sub} style={s.center}>{tr('No services yet — add your first one above.')}</T>}
        ListFooterComponent={services.length
          ? <T size={11} c={D.sub} style={s.footnote}>
              {tr('Hidden services stay on past bookings but customers can\'t pick them.')}
            </T>
          : null}
        renderItem={({ item }) => (
          <Pressable onPress={() => startEdit(item)} accessibilityRole="button"
            accessibilityLabel={tr('Edit {name}', { name: item.name })}
            ...
            <View style={s.grow}>
              ...
              <T size={11} c={D.sub} style={{ marginTop: 2 }}>
                {tr('{x} DH · {duration_min} min', { x: (item.price_cents / 100).toFixed(2), duration_min: item.duration_min })}
              </T>
            </View>
            {!item.is_active && (
              <View style={s.hiddenChip}><T w="b" size={9} c={D.sub} ls={0.7}>{tr('HIDDEN')}</T></View>
            )}
