-- 0057_appeal_desk_rename: undo an overload 0056 should never have created.
--
-- `admin_appeals()` has existed since 0045 and the console calls it on the
-- Reviews screen for the small "N appeals waiting" bar. 0056 added
-- `admin_appeals(p_appeal uuid default null)` for turn 3's desk — a different
-- shape under the same name, and because the new argument has a default, a
-- no-argument call matches **both**. PostgREST answers that with
-- "Could not choose the best candidate function", which would have broken the
-- Reviews screen the moment somebody opened it.
--
-- The desk gets its own name. Two functions that answer different questions
-- should not share one.

drop function if exists public.admin_appeals(uuid);
